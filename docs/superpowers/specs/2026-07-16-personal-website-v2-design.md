...
这两个变量由已连接的 Vercel Marketplace Supabase 集成注入；Astro/Vite 只额外允许 `NEXT_PUBLIC_` 前缀进入浏览器构建。浏览器代码不得读取或包含 Supabase secret/service-role key、数据库密码或直接 PostgreSQL 连接字符串。缺少公开配置时，互动组件显示“互动功能暂未配置”，静态网站和文章仍可正常访问。
